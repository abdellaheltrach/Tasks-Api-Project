// ===== CONFIGURATION =====
const API_BASE_URL = "/api"; // relative path works for both http/https
...
async function patchTaskStatus(id, statusString) {
  const statusId = getStatusId(statusString);
  const payload = { taskId: id, statusId: statusId };

  const response = await apiCall("PATCH", `/task/${id}/status`, payload);
  if (response !== null) {
    ...
    .map(
      (task) => `
        <div onclick="handleEditTask(${
          task.id
        })" class="bg-white rounded-xl p-6 shadow-sm hover:-translate-y-1 hover:shadow-md transition cursor-pointer border-l-4 border-black">
          <div class="flex justify-between items-start mb-4">
            <div class="flex-1">
              <h3 class="text-lg font-semibold mb-2">${escapeHtml(
                task.title
              )}</h3>
              <p class="text-gray-500 text-sm">${escapeHtml(
                task.description || "No description"
              )}</p>
            </div>
            <button onclick="event.stopPropagation(); handleDeleteTask(${
              task.id
            ...
            <div onclick="event.stopPropagation()">
                <select onchange="handleStatusChange(${task.id}, this.value)" 
                    class="px-2 py-1 rounded-full text-xs font-semibold uppercase border-none focus:ring-0 cursor-pointer ${
